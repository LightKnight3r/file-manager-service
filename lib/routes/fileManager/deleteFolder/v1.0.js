...
module.exports = (req, res) => {
  ...
  const getCurrentFolder = (next) => {
    ...
    .exec((err, result) => {
      ...
      // Kiểm tra quyền sở hữu - chỉ chủ sở hữu mới có quyền xóa
      if (result.ownerId && result.ownerId.toString() !== userId.toString()) {
        return next({
          code: CONSTANTS.CODE.ACCESS_DENINED,
          message: {
            head: 'Thông báo',
            body: 'Chỉ có chủ sở hữu thư mục mới có quyền xóa'
          },
        });
      }
