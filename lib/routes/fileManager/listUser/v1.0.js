...
module.exports = (req, res) => {
  ...
  let accessUsers = [];
  let folderInf = null;
  let parentAccessUsers = [];
  const listAccessUsers = (next) => {
    if(!folderId) {
      return next();
    }
    FolderModel
      .findById(folderId, 'accessUsers ownerId parentId')
      .lean()
      .exec((err, folder) => {
        if (err) {
          return next(err);
  ...
  const listAccessUsersParent = (next) => {
    if(!folderInf || !folderInf.parentId) {
      return next();
    }
    FolderModel
      .findById(folderInf.parentId, 'accessUsers ownerId')
      .lean()
      .exec((err, parentFolder) => {
        if (err) {
          return next(err);
        }
        if (!parentFolder) {
        ...
        // Gộp accessUsers của folder cha vào danh sách accessUsers
        parentAccessUsers = parentFolder.accessUsers || [];
        parentAccessUsers.push(parentFolder.ownerId);
        next();
      });
  }
  ...
  const listUsers = (next) => {
    ...
    if (edit === 1) {
      ...
      if(folderInf.parentId) {
        ...
        if (folderInf && parentAccessUsers.length > 0) {
          query._id.$in = parentAccessUsers;
        } else {
